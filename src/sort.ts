export const Sorts = {
	string: <T>(key?: T extends string ? string : keyof T, reverse = false) => (a: T, b: T) => {
		if (typeof a === "string") {
	...
	number: <T>(key?: T extends string ? string : keyof T, reverse = false) => (a: T, b: T) => {
		if (typeof a === "number") {
	...
	date: <T>(key?: T extends string ? string : keyof T, reverse = false) => (a: T, b: T) => {
		if (typeof a === "string") {
